...
const Expense = ({ amount, description, category, date }) => {
  return (
    <div className="flex items-center justify-between w-full p-2 text-black rounded-md">
      <span className="flex items-center justify-start h-full w-fit">{<Category category={category} />}</span>
      ...
      <span className="flex items-center gap-2 whitespace-nowrap">
        <span data-expense={amount<0} className="data-[expense=true]:text-red-600 text-green-600">
      {amount < 0 ? <CaretDown size={16} weight="fill" /> : <CaretUp  size={16} weight="fill" />}
        </span>
